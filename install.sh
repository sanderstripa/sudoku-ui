...
apt-get update -qq
apt-get install -y -qq ca-certificates curl jq qrencode tar iproute2 >/dev/null
...
if [[ "$PANEL_VERSION" == v0.1.* || "$PANEL_VERSION" == 0.1.* ]]; then
  "$BIN" --init --username "$USERNAME" --password "$PASSWORD" --listen ":$PORT" --repo "$REPO"
else
...
echo; echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"; echo
echo "Sudoku UI успешно установлен"; echo; echo "Панель:"; echo "http://${IP}:${PORT}${PANEL_SUFFIX}"
