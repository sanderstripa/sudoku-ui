...
Установщик не создаёт Sudoku-подключение. Он устанавливает стабильные релизы панели и Core, создаёт службы systemd, получает у Let's Encrypt короткоживущий сертификат непосредственно для публичного IP и печатает уникальный HTTPS-адрес, логин и пароль. Сертификат автоматически продлевается через `acme.sh`; внешний порт `80` и выданный порт панели должны быть открыты. Caddy, сторонний DNS и собственный домен не требуются. Если Sudoku уже найден, установщик спрашивает, использовать его, заменить бинарник с резервной копией или отменить установку.
...
Приватные ключи хранятся локально с правами `0600` и не возвращаются общим API состояния. Начиная с Sudoku UI v0.5.0 панель использует совместимую сборку Sudoku Core из релизов этого репозитория. Она поддерживает отключение и постоянный отзыв ключей, активные сессии, IP-адреса и раздельный учёт трафика через локальный API `/run/sudoku/core.sock`. Обновление Core больше не загружается напрямую из официального upstream.

Совместимая сборка основана на зафиксированном commit официального `SUDOKU-ASCII/sudoku`; применяемый патч находится в `core-patches/access-control.patch`. Core-релизы имеют теги `core-v*`, релизы панели — `v*`.
